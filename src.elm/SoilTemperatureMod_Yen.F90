...
contains
      ...
      do j = -nlevsno+1,nlevgrnd
         do fc = 1, num_nolakec
            ...
            if (snl(c)+1 < 1 .AND. (j >= snl(c)+1) .AND. (j <= 0)) then
               ...
               !!TJ - Testing three snow thermal conductivity scheme:Jordan (1991),Sturm et al.(1997), and Yen et al.(1965)
               !Original Snow Thermal Scheme: Jordan (1991) as in SNTHERM
               !thk(c,j) = tkair + (7.75e-5_r8 *bw(c,j) + 1.105e-6_r8*bw(c,j)*bw(c,j))*(tkice-tkair)
			   
			   !Sturm et al.(1997)
              !if ( bw(c,j)*0.001 > 0.156) then
              !    thk(c,j) = 0.138_r8 - 1.01_r8*(bw(c,j)*0.001_r8) + 3.233_r8*(bw(c,j)*0.001_r8)*(bw(c,j)*0.001_r8)
              !else
              !    thk(c,j) = tkair + 0.234_r8*(bw(c,j)*0.001_r8)
              !end if
              
              !!Yen1965, density in kg m-3
              thk(c,j) = 3.2217e-06_r8 * bw(c,j)*bw(c,j)
  ...
  subroutine Phasechange_beta (bounds, num_nolakec, filter_nolakec, dhsdT, &
    ...
    use elm_varpar       , only : nlevsno, nlevgrnd,nlevurb
    use elm_varctl       , only : iulog
    use elm_varcon       , only : denh2o, denice, tfrz, hfus, grav
	...
	integer ::  nstep
	real(r8) :: vsm_pec
	real(r8) :: eng_ch(bounds%begc:bounds%endc,-nlevsno+1:nlevgrnd)
	real(r8) :: eps(bounds%begc:bounds%endc,-nlevsno+1:nlevgrnd)	
	real(r8) :: eta_psi(bounds%begc:bounds%endc,-nlevsno+1:nlevgrnd)
	real(r8) :: supertemp(bounds%begc:bounds%endc,-nlevsno+1:nlevgrnd) 
    !-----------------------------------------------------------------------
      ...
      do j = -nlevsno+1,nlevgrnd       ! all layers
         do fc = 1,num_nolakec
            ...
            if (j >= snl(c)+1) then
               ...
               hm(c,j) = 0._r8
               xm(c,j) = 0._r8
               eps(c,j) = 1._r8 !default value is one.
               eng_ch(c,j) = 0._r8
               eta_psi(c,j) = 0._r8
