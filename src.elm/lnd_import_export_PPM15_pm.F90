...
contains
  ...
  subroutine lnd_import( bounds, x2l, atm2lnd_vars, glc2lnd_vars, lnd2atm_vars)
    ...
    integer  :: lnfmind(2)
    real(r8) :: ProbSnow !equation (1)&(2) in Jennings et al. (2019)
    real(r8) :: TotalPrecip, Tair, RHperc, Pskpa, qs, Tdew, Twet !Added by Jing Tao
...
10          continue
  ...
  !-----------------------------------------------------------------------------------------------------
       ...
       atm2lnd_vars%forc_lwrad_not_downscaled_grc(g) = x2l(index_x2l_Faxa_lwdn,i)    ! flwdsxy Atm flux  W/m^2
 	   ...
 	   !Already partitioned in: ./components/homme/utils/cime/src/share/util/shr_precip_mod.F90:  
	   !						subroutine shr_precip_partition_rain_snow_ramp(temperature, frac_rain)
	   !Called by ./E3SM_Master_Update/components/homme/utils/cime/src/components/data_comps/datm/mct/datm_comp_mod.F90:576: subroutine datm_comp_run
	   !frac_rain = (temperature - SHR_CONST_TKFRZ) * 0.5_r8
	   !Jing Tao - Defualt 50% rain-snow surface air temperature threshold = 1 Celsius degree; Label default PPM as PPM0;
       ...
       TotalPrecip = forc_rainc + forc_rainl + forc_snowc + forc_snowl
	   
       !write(iulog,"(A25,I4, 4(1X,F8.4))") 'Jing Tao: Original values=', g, forc_rainc, forc_rainl, forc_snowc, forc_snowl 
       
        !============================================
        !Calculate Relative Humidity (%)
        Tair = tdc(atm2lnd_vars%forc_t_not_downscaled_grc(g))
        if (Tair > 0.0) then
              e = esatw(Tair)
        else
              e = esati(Tair)
        ...
        qsat = 0.622_r8*e / (atm2lnd_vars%forc_pbot_not_downscaled_grc(g) - 0.378_r8*e)
        qs = atm2lnd_vars%forc_q_not_downscaled_grc(g)
        RHperc = min(100.0_r8, max(0.0_r8, 100.0_r8*(qs / qsat)))
        
        !Calculate dew point and wet-bulb temperature
        Tdew = 243.04*(log(RHperc/100)+((17.625*Tair)/(243.04+Tair)))/(17.625-log(RHperc/100)-((17.625*Tair)/(243.04+Tair)))
        Twet = Tair*atan(0.151977*(RHperc + 8.313659)**(0.5)) + atan(Tair + RHperc) - atan(RHperc - 1.676331) + 0.00391838*((RHperc)**(3.0/2.0)) * atan(0.023101*RHperc) - 4.686035
        
        !write(iulog,"(A10,I4,6(1X,F8.4))") 'Jing Tao:', g,Tair, qs, qsat, RHperc, Tdew, Twet
        
        !PPM1:
        !frac = Tair*0.5_R8 + 0.25_R8 
        
        !PPM2:
        !frac = Tair*0.25_R8 + 0.25_R8     
  
        !PPM3:
        !frac = Tair + 0.5_R8     
        
        !PPM4: Ta-based threshold
        !if (Tair > 0.0_R8) then
        !  frac = 1.0_R8
        !else
        !   frac = 0.0_R8
        ...
        !PPM8: Td(dew point)-based threshold
        !if (Tdew > 0.0_R8) then
        !	frac = 1.0_R8
        !else
        !    frac = 0.0_R8
        ...
        !PPM9: Td(dew point)-based threshold
        !if (Tdew > 1.0_R8) then
        !	frac = 1.0_R8
        !else
        !    frac = 0.0_R8
        ...
        !PPM10: Td(dew point)-based threshold
        !if (Tdew > 2.0_R8) then
        !	frac = 1.0_R8
        !else
        !    frac = 0.0_R8
        ...
        !PPM11: Tw(wet bulb)-based threshold
        !if (Twet > 0.0_R8) then
        !	frac = 1.0_R8
        !else
        !    frac = 0.0_R8
        ...
        !PPM12: Tw(wet bulb)-based threshold
        !if (Twet > 1.0_R8) then
        !	frac = 1.0_R8
        !else
        !    frac = 0.0_R8
        ...
        !PPM13: Tw(wet bulb)-based threshold
        !if (Twet > 2.0_R8) then
        !	frac = 1.0_R8
        !else
        !    frac = 0.0_R8
        ...
        !PPM14: RegBi - function of Ta and RH 
        ProbSnow = 1.0_R8 / (1+exp(-10.04+1.41*Tair+0.09*RHperc))
        !if (ProbSnow < 0.5_R8) then
        !	frac = 1.0_R8
        !else
        !   frac = 0.0_R8
        !end if
		
        !PPM15: RegBi - function of Ta and RH
		frac = 1.0_R8 - ProbSnow
						
		!write(iulog,"(A30,F8.4)") 'Jing Tao: RegBi =', ProbSnow
			
		!PPM16: RegTri - function of Ta, RH, and Ps
        !Pskpa = atm2lnd_vars%forc_pbot_not_downscaled_grc(g) * 0.001_R8 !in kPa
        !ProbSnow = 1.0_R8 / (1+exp(-12.8+1.41*Tair+0.09*RHperc+0.03*Pskpa))
        !if (ProbSnow < 0.5_R8) then
        !	frac = 1.0_R8
        !else
        !    frac = 0.0_R8
        ...
        !PPM17: RegTri - function of Ta, RH, and Ps
        !frac = 1.0_R8 - ProbSnow
        
        
        !write(iulog,"(A30,F8.4)") 'Jing Tao: RegTri=', ProbSnow
        
        frac = min(1.0_R8,max(0.0_R8,frac))           ! bound in [0,1]
        
        forc_rainc = TotalPrecip * frac *0.1_R8   ! mm/s
        forc_rainl = TotalPrecip * frac *0.9_R8   ! mm/s
        forc_snowc = TotalPrecip * (1-frac) *0.1_R8   ! mm/s
        forc_snowl = TotalPrecip * (1-frac) *0.9_R8   ! mm/s
